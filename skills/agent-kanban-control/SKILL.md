...
7. Reusable TODO lists are project-level modules. List them with `GET /api/projects/{projectId}/wiki/todo-lists`, create a unique name with `POST /api/projects/{projectId}/wiki/todo-lists`, append items with `POST /api/wiki-todo-lists/{listId}/items`, and update item text or completion through `PATCH /api/wiki-todo-items/{itemId}` using its latest `updatedAt` as `expectedUpdatedAt`. Embed only the stable reference `:::todo-list {#<list-uuid> label="<fallback-name>"} :::` in page Markdown; never copy the list's mutable items into the page.
